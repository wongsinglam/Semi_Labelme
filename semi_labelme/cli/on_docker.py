...
def semi_labelme_on_docker(in_file, out_file):
    ip = get_ip()
    cmd = "xhost + %s" % ip
    ...
    cmd += " wkentaro/semi_labelme semi_labelme {0}".format(in_file_b)
    if out_file:
...
def main():
    ...
    try:
        out_file = semi_labelme_on_docker(args.in_file, args.output)
        if out_file:
